...
@register_adv_est(AdvantageEstimator.RFPO)  # or simply: @register_adv_est("rfpo")
def compute_rfpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    old_log_probs: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    rrf_k = int(config.get("rrf_k", 1.0))
    tail_frac = float(config.get("tail_frac", 0.1))
    metric_weights = config.get(
        "metric_weights",
        {
            "mean_logprob": 1.0,
            "tail_mean_logprob": 1.0,
            "min_logprob": 0.5,
        },
    )
    
    # Confidence
    mean_logprob = verl_F.masked_mean(old_log_probs, response_mask, axis=-1)
    tail_mean_logprob = verl_F.masked_tail_mean_low(old_log_probs, response_mask, tail_frac=tail_frac)

    masked_lp = old_log_probs.masked_fill(~response_mask.bool(), float("inf"))
    min_logprob = masked_lp.min(dim=-1).values

    uid2rows = defaultdict(list)
    bsz = mean_logprob.shape[0]
    for i in range(bsz):
        uid2rows[index[i]].append(i)

    quality_scores = mean_logprob.new_zeros((bsz,), dtype=torch.float32)

    for uid in uid2rows:
        rows = torch.tensor(uid2rows[uid], device=mean_logprob.device, dtype=torch.long)
        group_mean_logprob = mean_logprob[rows].to(torch.float32)
        group_tail_mean_logprob = tail_mean_logprob[rows].to(torch.float32)
        group_min_logprob = min_logprob[rows].to(torch.float32)

        metric2scores = {
            "mean_logprob": group_mean_logprob,
            "tail_mean_logprob": group_tail_mean_logprob,
            "min_logprob": group_min_logprob,
        }

        metric2rank = {}
        for name, scores in metric2scores.items():
            if float(metric_weights.get(name, 0.0)) == 0.0:
                continue
            metric2rank[name] = verl_F._rank_desc(scores)

        if len(metric2rank) == 0:
            group_quality = torch.zeros((rows.numel(),), device=mean_logprob.device, dtype=torch.float32)
        else:
            group_quality = verl_F._rrf_fuse(metric2rank, metric_weights, k=rrf_k)

        quality_scores[rows] = group_quality

    grpo_scores = token_level_rewards.sum(dim=-1)
    ...
    with torch.no_grad():
        bsz = grpo_scores.shape[0]
        for i in range(bsz):
            id2score[index[i]].append(grpo_scores[i])
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                grpo_scores[i] = (grpo_scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            else:
                grpo_scores[i] = grpo_scores[i] - id2mean[index[i]]

    scores = grpo_scores * quality_scores
    scores = scores.unsqueeze(-1) * response_mask
